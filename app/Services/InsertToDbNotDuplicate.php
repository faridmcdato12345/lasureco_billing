...
class InsertToDbNotDuplicate {
    ...
    public function __construct($collection){
        ...
        $data->s_or_amount = floatval(Crypt::decryptString($collection['s_or_amount']));
        ...
        $data->e_wallet_added= floatval(Crypt::decryptString($collection['e_wallet_added']));
        $data->ap_status= (empty(Crypt::decryptString($collection['ap_status']))) ? null : Crypt::decryptString($collection['ap_status']);
		$data->s_cutoff= 1;
        $data->server_added= 1;
        $data->save();
        if($data){
        	$consId = Consumer::select('cm_id','temp_cons_id')
            ->where('temp_cons_id',intval(Crypt::decryptString($collection['cm_id'])))
            ->first();
            if(!is_null($consId)){
                Sales::where('cm_id',$data->cm_id)->update(['cm_id' => $consId['cm_id']]);
            }
            $chequeId = Cheque::select('cheque_id','temp_cheque_id')
            ->where('temp_cheque_id',intval(Crypt::decryptString($collection['cheque_id'])))
            ->first();
            if(!is_null($chequeId)){
                Sales::where('cheque_id',intval(Crypt::decryptString($collection['cheque_id'])))->update(['cheque_id' => $chequeId['cheque_id']]);
                Cheque::where('temp_cheque_id','>',0)->update(['temp_cheque_id' => 0]);
            }
            ...
            if(floatval($data->e_wallet_applied) != floatval('0') && floatval($data->e_wallet_added) == floatval('0')){
                ...
                $query->save();
                if($query){
                    $this->eWalletLogApplied($query->ew_id,$ewalletApplied,$orNumber,$data->s_bill_date);
                }
            }
            if(floatval($data->e_wallet_added) != floatval('0') && floatval($data->e_wallet_applied) == floatval('0')){
                $totalEwallet = floatval(Crypt::decryptString($collection['e_wallet_added']));
                $query = EWALLET::where('cm_id',intval(Crypt::decryptString($collection['cm_id'])))->first();
                $query->ew_total_amount += $totalEwallet;
                $query->save();
                if($query){
                    $this->ewalletLogAdded($query->ew_id,$totalEwallet,$orNumber,$data->s_bill_date);
                }
            }
