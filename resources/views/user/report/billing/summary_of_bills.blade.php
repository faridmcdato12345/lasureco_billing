...
<p class="contentheader">Summary of Bills</p>
<div class="main">
    <table id="maintable" class="content-table">
        ...
        <tr>
            <td colspan=2>
                <input type="checkbox" id="withDemand" name="withDemand">
                &nbsp; <label for="withDemand" id="recapLabel"> with demand? </label>
            </td>
        </tr>
    </table>
...
<script>
    ...
    function printSummBill(){
        ...
        var cap = "";
        var demand = "";
        var withDemand = document.querySelector('#withDemand');
        // console.log(withDemand);
        if(withDemand.checked == true){
            demand = 1;
        }
        else{
            demand = 0;
        ...
        $url = '{{route("print_summary_of_bills")}}'
        window.open($url);
    }
</script>
@endsection
