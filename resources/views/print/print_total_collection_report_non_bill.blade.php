...
<head>
    ...
    <title> Print Total Collection Report (Non-Bill) </title>
</head>
<style media="print">
    @page {
        size: A4;
...
<script>
    ...
    function getData(){
        ...
        var totalCollection = "{{route('collection.report.nonbill')}}";
        ...
        xhr.onload = function(){
            if(xhr.status == 200){
                ...
                output += '<label style="font-size: 20px;"> TOTAL COLLECTION REPORT (Non-Bill) </label> <br>';
                ...
                for(var i in details){
                    ...
                    if(num > 0 && num % 50 == 0){
                        ...
                        output += '</tr>';
                        total += details[i].Total_Collection;
                    }
                    else{
                        output += '<tr>';
                        output += '<td class="left">' + i + '</td>';
                ...
                output += "<td class='left top'>" + total.toLocaleString("en-US", { minimumFractionDigits: 2}) + "</td>";
                output += "</tr></table>";
